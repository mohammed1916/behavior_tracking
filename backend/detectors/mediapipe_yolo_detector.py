"""MediaPipe + YOLO detector combining object detection and motion analysis"""
...
class MediaPipeYoloStrategy(Enum):
    """How to combine signals from MediaPipe and YOLO detectors"""
...
class MediaPipeYoloDetector(DetectorBase):
    """
    ...
    def __init__(
        ...
        strategy: MediaPipeYoloStrategy = MediaPipeYoloStrategy.CASCADE,
        weights: Optional[Dict[str, float]] = None,
        device: Optional[str] = None,
    ):
        """
        Initialize MediaPipe + YOLO detector.
        ...
        """
        super().__init__(name='mediapipe_yolo', confidence_threshold=0.5)
        
        self.strategy = strategy if isinstance(strategy, MediaPipeYoloStrategy) else MediaPipeYoloStrategy[strategy.upper()]
    ...
    def process_frame(self, frame) -> DetectorOutput:
        ...
        # Fuse both signals
        if self.strategy == MediaPipeYoloStrategy.CASCADE:
            return self._fuse_cascade(mediapipe_result, yolo_result)
        elif self.strategy == MediaPipeYoloStrategy.WEIGHTED:
            return self._fuse_weighted(mediapipe_result, yolo_result)
        elif self.strategy == MediaPipeYoloStrategy.CONSENSUS:
            return self._fuse_consensus(mediapipe_result, yolo_result)
        elif self.strategy == MediaPipeYoloStrategy.MAJORITY_VOTE:
